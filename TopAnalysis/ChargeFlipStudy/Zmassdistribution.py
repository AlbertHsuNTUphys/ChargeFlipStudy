...
# Histograms
h_os_Zmass_MC = ROOT.TH1F('os_Zmass_MC',"; M(l,l) [GeV] ; Events",150,0,150)
h_os_Zmass_data = ROOT.TH1F('os_Zmass_data',"; M(l,l) [GeV] ; Events",150,0,150)
...
diagrams = ['ee_ss_Zmass', 'ee_os_Zmass']
Zmass_result = dict()
sig_list = ["DY"]
...
for s,desc in samplesList:
  ...
  for ff in AllFiles:
    if s in ff:
      ...
      try:
        h_os = inf.Get('ee_os_Zmass')
        h_ss = inf.Get('ee_ss_Zmass')
        if ("MC" in ff and (desc[3] in sig_list) and not apply_SF):
          print("signal"+ff)
          h_os_Zmass_MC.Add(h_os_Zmass_MC,h_os,1.0,weight)
          h_ss_Zmass_MC.Add(h_ss_Zmass_MC,h_ss,1.0,weight)
        elif("Data" in ff):
          h_os_Zmass_data.Add(h_os_Zmass_data,h_os,1.0,1.0)
          h_ss_Zmass_data.Add(h_ss_Zmass_data,h_ss,1.0,1.0)
          #else:
          #  h_os_Zmass_data.Add(h_os_Zmass_data,h_os,1.0,-1.*weight)
...
numberlist[2] = h_os_Zmass_data.Integral()
numberlist[3] = h_os_Zmass_MC.Integral()

#MC_SF = (numberlist[0]+numberlist[2])/(numberlist[1]+numberlist[3])


h_os_Zmass_MC.GetXaxis().SetRange(70,110)
#h_os_Zmass_MC.Scale(MC_SF)
h_os_Zmass_MC.Draw("HIST")
c.Update()

h_os_Zmass_data.GetXaxis().SetRange(70,110)
h_os_Zmass_data.SetMarkerStyle(8)
h_os_Zmass_data.Draw("E2 SAME")
c.Update()

print(h_ss_Zmass_MC.GetMaximum())

rightmax = 1.7 * float(h_ss_Zmass_MC.GetMaximum())
...
h_ss_Zmass_data.SetMarkerStyle(8)
...
legend.AddEntry(h_os_Zmass_data,"Data(OS)","p")
legend.AddEntry(h_os_Zmass_MC,"MC(OS)","l")
legend.Draw("SAME")
...
w.factory('BreitWigner::BW(x[60,120],m[90,70,110],#Gamma[0,8.])')
...
h_list = [h_ss_Zmass_data,h_ss_Zmass_MC,h_os_Zmass_data,h_os_Zmass_data]
...
print("ss data ; ss MC ; os data; os MC")
