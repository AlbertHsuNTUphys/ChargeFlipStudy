  ...
  for j in range(eta_bins):
    for ii in range(pt_bins):
      for jj in range(eta_bins):
        ...
        print(str(Number['data_oc'][i][j][ii][jj])+" "+str(Number['MC_oc'][i][j][ii][jj]))
  ...
  def fcn(npar, gin, f, par, iflag):
      ...
      for i in range(pt_bins):
        for j in range(eta_bins):
          for ii in range(pt_bins):
            for jj in range(eta_bins):
              if (not Vij[h][i][j][ii][jj]==0.) and Number[h+'_oc'][i][j][ii][jj]>200. and Number[h+'_ss'][i][j][ii][jj]>1.:
                P1 = par[i*eta_bins+j]
