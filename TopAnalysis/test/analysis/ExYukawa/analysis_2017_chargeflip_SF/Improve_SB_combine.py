...
plotdir = "/eos/user/t/tihsu/DataAnalysis/trial/"
...
# Basic Setting
pt_region = np.array(( 20.0, 50.0, 100.0, 200.0, 300.))
  ...
  def fcn(npar, gin, f, par, iflag):
      ...
      for i in range(pt_bins):
        for j in range(eta_bins):
          for ii in range(pt_bins):
            for jj in range(eta_bins):
              if (not Vij[h][i][j][ii][jj]==0.) and Number[h+'_oc'][i][j][ii][jj]>30 and Number[h+'_ss'][i][j][ii][jj]>0:
                P1 = par[i*eta_bins+j]
