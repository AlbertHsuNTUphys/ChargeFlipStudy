...
def prepare_shell(name,sample_type,year,era,subdir,inDir,outDir,site, FarmDir,condor,total_shell,File):
...
#    subprocess.Popen(f'rm -rf {FarmDir}', shell=True,stdout=subprocess.PIPE)
    ...
    for dataset in jsons:
        prepare_shell(dataset['name'], dataset['type'], str(dataset['year']), dataset['era'], dataset['dir'], args.inDir, args.outDir,args.site,FarmDir,condor,total_shell,dataset['file'])
